...
export const useDiagram = (options?: UseDiagramOptions): UseDiagramReturn => {
  ...
  >(undefined);
  const flowWrapperRef = useRef<HTMLDivElement>(
    null,
  ...
  const normalizeSnapshot = (snap: Snapshot) => {
    const nodes = [...(snap.nodes ?? [])].sort((a: any, b: any) =>
      String(a.id).localeCompare(String(b.id)),
    );
    const edges = [...(snap.edges ?? [])].sort((a: any, b: any) =>
      String(a.id).localeCompare(String(b.id)),
    );
    const vp = snap.viewport as Viewport | undefined;
    const viewport = vp
      ? { x: vp.x ?? 0, y: vp.y ?? 0, zoom: vp.zoom ?? 1 }
      : undefined;
  ...
  const takeSnapshot = useCallback(
    (
      overrideNodes?: DiagramNode[],
      overrideEdges?: DiagramEdge[],
    ): Snapshot => {
      if (rfInstance) {
  ...
  const pushSnapshot = useCallback(
    (overrideNodes?: DiagramNode[], overrideEdges?: DiagramEdge[]) => {
  ...
  const applySnapshot = useCallback(
    (snap: Snapshot | undefined) => {
      ...
      const nextNodes = (
        Array.isArray(snap.nodes) ? snap.nodes : []
      ) as DiagramNode[];
      const nextEdges = (
        Array.isArray(snap.edges) ? snap.edges : []
      ) as DiagramEdge[];

      // Validate node structure - ensure all nodes have required fields
      const validNodes = nextNodes.filter(
        (n) =>
          n &&
          n.id &&
          n.type &&
      ...
      const validEdges = nextEdges.filter(
  ...
  const loadSaved = useCallback((): ReactFlowJsonObject<
    DiagramNode,
    DiagramEdge
  > | null => {
    ...
    try {
      ...
      return JSON.parse(raw) as
        | ReactFlowJsonObject<DiagramNode, DiagramEdge>
        | ReactFlowJsonObject<DiagramNode, DiagramEdge>[] as any;
    } catch (e) {
  ...
  const saveDiagram = useCallback(async (): Promise<boolean> => {
    ...
    const viewportPayload = viewport
    ...
    if (useBackend && getToken) {
      ...
      if (diagramId) {
        ...
        loadById(diagramId).catch((e) => {
          if (!cancelled)
            console.warn("Failed to load diagram from backend", e);
        ...
        listDiagrams(token)
          .then(({ diagrams: list }) => {
            ...
            if (mostRecent) {
              setAccessLevel(
                (mostRecent as { accessLevel?: DiagramAccessLevel })
                  .accessLevel ?? "owner",
              );
  ...
  const exportToJson = useCallback((): string | null => {
    ...
    const flow: ReactFlowJsonObject<DiagramNode, DiagramEdge> =
      rfInstance.toObject();
  ...
  const onUpdateNode = useCallback(
    (nodeId: string, data: Partial<import("@/types/diagram").NodeData>) => {
      setNodes((nds) => {
  ...
  const onUpdateEdge = useCallback((edgeId: string, data: any) => {
    ...
    setSelectedEdge((current) =>
      current?.id === edgeId ? { ...current, ...data } : current,
    );
  }, []);
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      if (data.type === "node" && data.nodeType) {
        ...
        const size = NODE_DEFAULT_SIZE[data.nodeType] ?? {
          width: 80,
          height: 60,
        };
        ...
        setNodes((nds) => {
          ...
          const parents = updatedNodes.filter(
            (n) => !n.parentId || n.type === "securityRealmNode",
          );
      ...
      setNodes((ns) =>
        ns.map((n) => {
          const isIntersecting =
            intersections.includes(n.id) && n.id !== node.id;
      ...
      setNodes((ns) => {
        ...
        if (currentNode.type === "securityRealmNode") {
          ...
          if (!securityRealmParent && currentNode.position) {
            ...
            if (currentNode.parentId) {
              const oldParent = currentNodes.find(
                (p) => p.id === currentNode.parentId,
              );
            ...
            const manualCheck = currentNodes.find((n) => {
              ...
              return (
                (absX >= n.position.x &&
                  absX <= n.position.x + n.width &&
                  absY >= n.position.y &&
                  absY <= n.position.y + n.height) ||
                (absX + nodeWidth >= n.position.x &&
                  absX + nodeWidth <= n.position.x + n.width &&
                  absY + nodeHeight >= n.position.y &&
                  absY + nodeHeight <= n.position.y + n.height) ||
                (absX + nodeWidth / 2 >= n.position.x &&
                  absX + nodeWidth / 2 <= n.position.x + n.width &&
                  absY + nodeHeight / 2 >= n.position.y &&
                  absY + nodeHeight / 2 <= n.position.y + n.height)
              );
            });
        ...
        setTimeout(() => {
          ...
          if (
            ...
            latestEdges &&
            Array.isArray(latestEdges)
          ) {
            saveToStorage(
              latestNodes as DiagramNode[],
              latestEdges as DiagramEdge[],
            );
          }
        }, 100);
